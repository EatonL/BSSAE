...
def run2(i):
    with LOCK:
        print(i)
        if not os.path.exists(osp.join(savepath_val, f'{Y[i, 0]}')):
            os.makedirs(osp.join(savepath_val, f'{Y[i, 0]}'))
        dataX = DWT_hstack(X[i])
        np.save(osp.join(savepath_val, f'{Y[i, 0]}/{b[Y[i, 0]]}_{Z[i, 0]}.npy'), dataX)
        b[Y[i, 0]] += 1
    # print(a)
...
if __name__ == '__main__':
    ...
    # exit()
    #
    filename = r'/root/autodl-tmp/Pretrain/2018.01.OSC.0001_1024x2M.h5/2018.01/GOLD_XYZ_OSC.0001_1024.hdf5'
    savepath = r"/root/autodl-tmp/Pretrain/dataset2018_obo"
